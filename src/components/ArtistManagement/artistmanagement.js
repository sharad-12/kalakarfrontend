...
const ArtsistManagement = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await fetch("https://api.ekalakaar.com/api/v1/admin/user?role=Artist", {
          method: "GET",
          headers: {
            Authorization:
              "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NTQwODJjYzU1NzBkODY1NjA2NGVmNjciLCJpYXQiOjE3MDQ0NjAyNTQsImV4cCI6MTcwNDU0NjY1NH0.3bC-O4JIaVqgKmsQzpZadK0I5qBpSFSlW1vmNxb5wnM",
            "Content-Type": "application/json",
          },
        });
  ...
  return (
    <>
      ...
      <div className="artist_top_buttons">
        ...
        <button className="total_artist">
          Total Artist <br />
          <span style={{ color: "black", fontStyle: "bold" }}> {data.length}</span>
        </button>
        <div className="total_artist">
      ...
      <ReactPaginate
        ...
        pageCount={Math.ceil(filteredProfiles.length / itemsPerPage)}
