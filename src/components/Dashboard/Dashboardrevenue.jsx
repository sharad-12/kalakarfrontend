...
export default function Dashboard() {
  ...
  const[totalArtLover, setTotalArtLover]=useState(0);
  const [toatlPartner, setTotalPartner] = useState(0);
  ...
  useEffect(() => {
    const getUser = async (role) => {
      try {
        ...
        else if(role==="ArtLover"){
          setTotalArtLover(responseData.data.length);
        }
        else if(role==="Partner"){
          setTotalPartner(responseData.data.length);
        }
      } catch (error) {
